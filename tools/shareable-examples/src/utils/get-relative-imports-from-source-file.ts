...
import { SourceFile, isImportDeclaration, ImportDeclaration } from 'typescript';
...
export async function getRelativeImportsFromSourceFile(
  ...
  const relativeImports = source.statements
    ...
    .map((statement: ImportDeclaration) => statement.moduleSpecifier.getText())
    ...
    .map((specifier: string) => specifier.replace(/'/g, ''));
