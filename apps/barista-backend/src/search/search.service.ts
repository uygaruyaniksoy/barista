...
export class SearchService {
  ...
  private _documentCache: Map<
    string,
    { timestamp: number; data: BaPageBuilderContentResult }
  > = new Map();
  ...
  private _getContentSource(
    path: string,
  ): Observable<BaPageBuilderContentResult> {
    const now = Date.now();
    ...
    return this._http
      .get<BaPageBuilderContentResult>(
        `${environment.dataHost}/${sanitizedPath}.json`,
      )
      .pipe(
